...
def create_unique_drone_model(source_model_path, drone_id):
    ...
    if os.path.exists(config_path):
        create_config_file(config_path, drone_id)
...
def create_config_file(config_path, drone_id):
    """
    Создает новый файл конфигурации модели
...
def main():
    if len(sys.argv) < 2:
        print("Использование: python create_drone_models.py <путь_к_модели_iris_with_ardupilot>")
        print("Например: python create_drone_models.py /home/timowko/ardupilot_gazebo/models/iris_with_ardupilot")
        return
